...
static void setup_wallpaper_for_output(struct tinywl_output *output) {
	struct tinywl_server *server = output->server;

	struct wlr_box output_box = { 0 };
	wlr_output_layout_get_box(server->output_layout, output->wlr_output, &output_box);

	float color[4] = { 0.12f, 0.24f, 0.47f, 1.0f };

	server->wallpaper_rect = wlr_scene_rect_create(
		&server->scene->tree, 
		output_box.width, 
		output_box.height, 
		color
	);
	server->wallpaper_node = &server->wallpaper_rect->node;

	wlr_scene_node_set_position(server->wallpaper_node, output_box.x, output_box.y);
	wlr_scene_node_lower_to_bottom(server->wallpaper_node);
}
...
static void output_frame(struct wl_listener *listener, void *data) {
	...
	// Handle renderer drawing
	g_dock_on_render_pass(output->dock_panel, pass);
...
static void server_new_output(struct wl_listener *listener, void *data) {
	...
	// Setup wallpaper
	setup_wallpaper_for_output(output);

	// Setup dock panel
	g_dock_panel_create(output);
	
	// Create dock panel apps
	...
	struct g_dock_app *weston_terminal = g_dock_app_create(output, "weston-terminal", "Terminal", "/usr/bin/weston-terminal");

	// Add apps to dock panel
	g_dock_panel_add_application(output, firefox_app);
	g_dock_panel_add_application(output, gimp_app);
	g_dock_panel_add_application(output, weston_terminal);
}
...
static void xdg_popup_destroy_notify(struct wl_listener *listener, void *data) {
